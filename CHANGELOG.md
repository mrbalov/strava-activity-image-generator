...
- Updated imports in server routes to use the new external package with simplified naming (removed "Strava" prefix from function and constant names)

### Removed
- Internal `packages/get-strava-activity-image-generation-prompt` package from monorepo (functionality now provided by external npm module)
...
### Removed
- Internal `packages/get-strava-activity-signals` package from monorepo (functionality now provided by external npm module)
